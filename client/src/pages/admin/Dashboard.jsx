...
const Dashboard = () => {
  console.log("Dashboard Rendering - CLIENT_ADMIN_V3.0");
  ...
  if (data?.error === "FORBIDDEN") {
    return (
      <div className="p-12 bg-rose-50 border border-rose-100 rounded-3xl text-center">
        <h2 className="text-xl font-bold text-rose-800 mb-2">Access Denied (403)</h2>
        <p className="text-rose-600 font-medium text-sm mb-6">
          You are currently logged in as a Customer. Only Administrators can access this dashboard.
        </p>
        <div className="bg-white p-6 rounded-2xl border border-rose-100 inline-block text-left mb-6 shadow-sm">
           <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Use Admin Credentials:</p>
           <p className="text-sm font-bold text-slate-700">Email: <span className="text-blue-600 font-mono">admin@mscaterers.com</span></p>
           <p className="text-sm font-bold text-slate-700">Password: <span className="text-blue-600 font-mono">admin123</span></p>
        </div>
        <br />
        <button 
          onClick={() => { localStorage.clear(); window.location.href = '/admin/login'; }}
          className="bg-rose-600 text-white px-8 py-3 rounded-xl font-bold text-sm shadow-lg shadow-rose-600/20 hover:bg-rose-700 transition-all"
        >
          Logout and Switch to Admin
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Bookings" value={data?.stats?.totalBookings || 0} icon={ShoppingBag} color="bg-blue-500" trend="12" trendType="up" />
        <StatCard title="Net Revenue" value={`₹${(data?.stats?.totalRevenue || 0).toLocaleString()}`} icon={IndianRupee} color="bg-emerald-500" trend="24" trendType="up" />
        <StatCard title="Total Customers" value={data?.stats?.userCount || 0} icon={Users} color="bg-indigo-500" trend="8" trendType="up" />
        <StatCard title="Pending Tasks" value={data?.stats?.pendingBookings || 0} icon={Clock} color="bg-amber-500" trend="5" trendType="down" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm">
          ...
          <div className="space-y-5">
            {(data?.recentBookings || []).slice(0, 5).map((booking, idx) => (
              <div key={idx} className="flex items-center justify-between group cursor-pointer hover:bg-slate-50/80 p-2 rounded-xl transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 text-[14px] tracking-tight leading-none">
